...
I actually did not miss anything.  Or perhaps I did have a problem somewhere.  At any rate, I deleted the mask and tried that part again.  The point where I thought I was having a problem was that I thought the image would resize like it did in the animation in the tutorial.  But the image just resizes like anything would resize.  I pressed on and it worked.  One thing that I would say though is that the heirachy of the canvas should be noted.  I took a [picture](https://github.com/MichaelTMiyoshi/RubyUnityTutorial/blob/main/Images/CanvasHeirarchy.png) of the canvas heirarchy (not the whole heirarchy).  The main thing to note is that the mask is at the same level as the image of Ruby in the Health heirarchy.

![CanvasHeirarchy](https://github.com/MichaelTMiyoshi/RubyUnityTutorial/blob/main/Images/CanvasHeirarchy.png)
